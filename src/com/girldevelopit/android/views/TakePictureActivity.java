...
public class TakePictureActivity extends Activity {
	...
	// when we take or select a photo, we have to do something with the
	// takepicture we get back
	// these integers are our way of knowing if we took a takepicture or
	// selected an existing takepicture
	private final int ACTIVITY_TAKE_PHOTO = 1;
	private final int ACTIVITY_SELECT_PHOTO = 2;

	// there are four elements in our layout that we will want to get data from,
	 ...
	 * functionality of that method. The default functionality is just to create
	 * an acitivity that the user can see. we want to do that AND make that
	...
	private void initElements() {
		pictureFromCamera = (ImageView) this
				.findViewById(R.id.pictureFromCamera);
	...
	public void takePicture(View view) {
		...
		intent.putExtra(MediaStore.EXTRA_OUTPUT,
				Uri.fromFile(Utils.getTempFile(getApplicationContext())));
	...
	/**
	 * requestCode -- which activity/intent was this?? resultCode -- did we
	 * fail? did we succeed? data - contains a Map of stuff! Like file paths to
	 * our photo!
	 */
	@Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(requestCode == ACTIVITY_TAKE_PHOTO){
			try {
				pathToImage = Utils.getTempFile(getApplicationContext())
						.getAbsolutePath();
				...
				try {
					...
					thumbnailBmp.compress(Bitmap.CompressFormat.JPEG, 100,
							ostream);
					ostream.close();
				} catch (Exception exp) {
					Log.e("TakePhoto", "Thumbnail conversion error: " + exp.getMessage());
				}
				System.gc();
			} catch (Exception exp) {
		...
		if(requestCode == ACTIVITY_SELECT_PHOTO){
			try{
				System.gc();
				pathToImage = Utils.getPath(data.getData(),
						getContentResolver());
				...
				Log.e("SelectPhoto", exp.getMessage());
			}
		}
	}
